...
function FluentButton.New(options)
    ...
    local TweenButtonScale = function(targetScale, duration, easing)
    ...
    for _, InputType in ipairs({Enum.UserInputType.MouseButton1, Enum.UserInputType.Touch}) do
        ImageButton.InputBegan:Connect(function(input)
            if ImageButton.Active and input.UserInputType == InputType and not Active then
                Active = true
                TweenButtonScale(0.9, 0.065, Enum.EasingStyle.Circular)
            end
        end)
        ImageButton.InputEnded:Connect(function(input)
            if ImageButton.Active and input.UserInputType == InputType and Active then
                Active = false
